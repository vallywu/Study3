...
#	pdb.set_trace()
	...
	for analyte in quantData:
		...
		for element in analyte.findall('VALUE'):
			...
			refDict = dict()
			if (reference is not None) & ('unit' in element.attrib.keys()):
			...
			elif (reference is not None) & ('concUnit' in element.attrib.keys()):
				if (element.attrib['concUnit'] == reference.attrib['unit']):
					# Implicitly 2.5 and 97.5 in lipo xml
					refDict['Lower Reference Bound'] = 2.5
					refDict['Upper Reference Bound'] = 97.5
			...
			except: # NEW BI-QUANT (BI-QUANT-PS 2.0)
				reldata = analyte.find('RELDATA') # Amended to extract rawConc as value
				item = {'Feature Name': name,
						'comment': comment,
						'type': qtype,
						'value': element.attrib['conc'], # 'value': reldata.attrib['rawConc'],
						'lod': to_numeric(element.attrib['lod']),
						'loq': to_numeric(element.attrib['loq']),
						'Unit': element.attrib['concUnit']}
